...
class GuardrailAction(BaseModel):
    message: str = Field(description="The message to send to the environment")
    redacted_text: str = Field(default="", description="The redacted version of the text")
    safe_response: str = Field(default="", description="The text with the secret [REDACTED]")
...
class State(BaseModel):
    model_config = ConfigDict(frozen=False)  # allow field mutation (step_count += 1)
